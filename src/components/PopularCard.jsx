    ...
    return (
      ...
      <section>
        <div className="flex flex-col items-center mb-12 space-y-2">
            <span className="text-orange-500 font-semibold tracking-widest uppercase text-xs animate__animated animate__fadeIn">Summer Essentials</span>
            <h2 className="text-4xl md:text-5xl font-black text-slate-900 text-center animate__animated animate__fadeInDown">Popular Products</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {popularProducts.slice(0, 3).map((product) => (
            <Card 
                key={product.id} 
                className="group border-none bg-slate-50/50 hover:bg-white transition-all duration-500 hover:shadow-[0_20px_50px_rgba(0,0,0,0.08)] overflow-hidden"
                shadow="none"
            >
              ...
              <CardFooter className="flex flex-col items-start p-6 gap-4">
                <div className="flex justify-between items-start w-full">
                  <div>
                    ...
                    <p className="text-slate-400 text-sm mt-1 uppercase tracking-tighter">Premium Collection</p>
                  </div>
                  <p className="text-2xl font-black text-orange-500">${product.price}</p>
                </div>

                <Link href={`/all-products/${product.id}`} className="w-full">
                    <Button 
                        fullWidth 
                        className="bg-slate-900 hover:bg-orange-500 text-white font-bold py-6 rounded-xl transition-colors duration-300 shadow-lg shadow-slate-200"
                    >
                        View Details
                    </Button>
                </Link>
      ...
      <section>
        <div className="flex items-center gap-6 mb-12">
            <h2 className="text-2xl font-black text-slate-400 whitespace-nowrap uppercase tracking-widest">Trust the Best</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {['SunShade', 'GlowGuard', 'AquaFlow', 'SummerVibe'].map((brand) => (
            <Card 
                key={brand} 
                className="flex items-center justify-center p-10 bg-white border border-slate-100 shadow-none hover:border-orange-200 transition-all duration-300 grayscale hover:grayscale-0"
            >
              <span className="text-2xl font-black text-slate-300 group-hover:text-slate-900 uppercase italic tracking-tighter transition-colors">
                {brand}
              </span>
            </Card>
          ))}
